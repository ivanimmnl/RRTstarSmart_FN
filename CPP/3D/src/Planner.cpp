...
Planner::Planner(Map_manager manager, int step_size)
{
    ...
    branch_length = step_size*2;
    region_radius = step_size*3;
    beacon_radius = step_size;
...
std::vector<float> Planner::get_random_point()
{
    ...
    std::uniform_real_distribution<> x(0, voxel_dim-1);
    std::uniform_real_distribution<> y(0, voxel_dim-1);
    std::uniform_real_distribution<> z(0, voxel_dim-1);
    floatVec.push_back(x(gen));
    floatVec.push_back(y(gen));
    floatVec.push_back(z(gen));
...
{
    //int random = randNum(0, Cfree.size());
    float theta = M_PI * randNum(1, 100)/100;
    float psi = 2* M_PI * randNum(1, 100)/100;
    float radius = beacon_radius * randNum(1, 100)/100;
    int j = randNum(0, Xbeacon_old.size() - 3) + 1;
    std::vector<float> floatVec;

    float x = floor(Xbeacon_old[j][0] + radius*sin(theta)*cos(psi));
    float y = floor(Xbeacon_old[j][1] + radius*sin(theta)*sin(psi));
    float z = floor(Xbeacon_old[j][2] + radius*cos(theta));

    floatVec.push_back(x);
    floatVec.push_back(y);
    floatVec.push_back(z);
...
bool Planner::hasObstacle(std::vector<float> Xnear, std::vector<float> Xnew)
{
    ...
    float diff3 = (Xnew[2] - Xnear[2] + 0.00001);

    int decimated_index_a = 1;
    int decimated_index_b = 2;
    float diff = diff1;
    ...
    if (fabs(diff3) > fabs(diff1) && fabs(diff3) > fabs(diff2))
    {
        diff = diff3;
        decimated_index_a = 0;
        decimated_index_b = 1;
    ...
    for (int ii = 1; ii <= fabs(diff); ii++)
    {
        ...
        std::vector<float> tempa,tempb;

        point.push_back(Xnear[0] + ii * diff1 / fabs(diff));
        point.push_back(Xnear[1] + ii * diff2 / fabs(diff));
        point.push_back(Xnear[2] + ii * diff3 / fabs(diff));

        tempa.push_back(point[decimated_index_a]);
        tempb.push_back(point[decimated_index_b]);
        point[decimated_index_a] = floor(point[decimated_index_a]);
        point[decimated_index_b] = floor(point[decimated_index_b]);

        points_to_check.push_back(point);

        if (tempa[0] != point[decimated_index_a])
        {
            point[decimated_index_a]=point[decimated_index_a]+1;
            points_to_check.push_back(point);
        }

        point[decimated_index_a]=point[decimated_index_a]-1;
        if (tempb[0] != point[decimated_index_b])
        {
            point[decimated_index_b]=point[decimated_index_b]+1;
            points_to_check.push_back(point);
        }
    }
    ...
    for (int jj = 0; jj < points_to_check.size(); jj++)
    {
        ...
        floatVec.push_back((int)(points_to_check[jj][0]));
        floatVec.push_back((int)(points_to_check[jj][1]));
        floatVec.push_back((int)(points_to_check[jj][2]) < 0? 0:(int)(points_to_check[jj][2]));
...
std::vector<float> Planner::new_node(std::vector<float> Xnear, std::vector<float> Xrand)
{
    ...
    float distance = calculateDistance(Xrand, Xnear);

    if(distance >= branch_length){
        Xnew.push_back(Xnear[0]+floor(((Xrand[0]-Xnear[0])*branch_length/distance)));
        Xnew.push_back(Xnear[1]+floor(((Xrand[1]-Xnear[1])*branch_length/distance)));
        Xnew.push_back(Xnear[2]+floor(((Xrand[2]-Xnear[2])*branch_length/distance)));
    }
    else{
        Xnew = Xrand;
    }
...
std::vector<float> Planner::get_best_parent(std::vector<int> neighbourhood)
{
    ...
    // The fourth index is the position in the tree
    return Xnear;
}
...
{
    return (float)sqrt((double)pow(first_point[0] - second_point[0], 2) + (double)pow(first_point[1] - second_point[1], 2) + (double)pow(first_point[2] - second_point[2], 2));
}
...
std::vector<geometry_msgs::PoseStamped> Planner::makePlan(std::vector<float> root, std::vector<float> target)
{
    ...
    floaterVec.push_back(floor(target[2]));
    ...
    while (count<30000)
    {
        ...
        if(Xnearest[0]==Xrand[0] || Xnearest[1]==Xrand[1] || Xnearest[2]==Xrand[2])
            continue;
        ...
        intVec.push_back(floor(Xnew[2]));

        if (map.checkObstacle(intVec)) continue; //if new node is in obstacle, continue with next iteration
        ...
        if (!hasObstacle(Xnear, Xnew))
        {
            ...
            if (init_path_found)
            {
                ...
                Xend.push_back(node[n_target][2]);
    ...
    for (int ii = 0; ii < Xbeacon_old.size(); ii++)
    {
        ...
        pos.pose.position.x = ((Xbeacon_old[ii][0]+1)*90.9341/(float)voxel_dim - 26.3329);
        pos.pose.position.y = ((Xbeacon_old[ii][1]+1)*90.9341/(float)voxel_dim - 25.4699);
        pos.pose.position.z = ((Xbeacon_old[ii][2]+1)*90.9341/(float)voxel_dim);
