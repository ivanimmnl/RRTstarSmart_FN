function [route, init_n, init_cost, final_cost] = RRTStar_SmartFN (numIterate, maxNodes, x_max, y_max, obstacle, q_start, q_goal, b)
%close all
...
%     i = i+1;
    ...
    if InitialPathFound && mod(i-init_n,b) == 0 %Spawn random point near beacon
        ...
        q_rand = [radius*cos(angle) radius*sin(angle)] + q_beacons_old{j};
        q_rand(1) = floor(q_rand(1));
        q_rand(2) = floor(q_rand(2));
        
        %continue iteration if sample is outside workspace
        if q_rand(1) <= 0 || q_rand(1) > x_max || q_rand(2) <= 0 || q_rand(2) > y_max
            continue
        end
    else
        q_rand = [randi([1 x_max], 1) randi([1 y_max], 1)];
    end
    ...
    if ObstacleFree(q_new, obstacle)
        ...
        if ~isinf(C_min)
            ...
            if ~InitialPathFound && abs(q_new(1) - q_goal(1)) <= 10 && abs(q_new(2) - q_goal(2)) <= 10
                q_goal = q_new;
                init_n = i;
%                 i = 1; %to reset
                n_goal = length(nodes);
                init_cost = q_new_cost;
                directcost_old = q_new_cost;
...
%             rectangle('Position',obstacle(j,:),'FaceColor',[0 .5 .5], 'LineStyle', 'none')
%         end
    
    plot(600, 100, 'rs', 'MarkerSize', 20)
    plot(q_start(1), q_start(2), '*', 'MarkerFaceColor', 'b', 'MarkerSize', 20) 
    
        for j = 2:1:length(nodes)
            child = nodes{j};
            parent = nodes{nodes_parent(j)};
            line([child(1), parent(1)], [child(2), parent(2)], 'Color', 'g');
        end
    ...
    for j = 1:1:length(q_beacons_old)-1
        ...
        route = [route; parent];
                line([child(1), parent(1)], [child(2), parent(2)], 'Color', 'r', 'LineWidth', 2);
                hold on
    end
